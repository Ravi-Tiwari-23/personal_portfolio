...
from app.media_storage import image_url
...
def install_fake_cloudinary(monkeypatch, calls):
    ...
    def upload(file_object, **kwargs):
        ...
        return {
            "secure_url": "https://res.cloudinary.com/demo/image/upload/portfolio/certificates/asset.png",
            "public_id": "portfolio/certificates/asset",
        }
...
def test_cloudinary_certificate_upload_stores_secure_url_and_public_id(app, client, auth, monkeypatch):
    ...
    with app.app_context():
        ...
        assert certificate.image_public_id == "portfolio/certificates/asset"
    upload_call = next(call for call in calls if call[0] == "upload")
    assert upload_call[1]["folder"] == "portfolio/certificates"
    assert b"test-secret" not in client.get("/").data
    assert b"https://res.cloudinary.com/demo/image/upload/portfolio/certificates/asset.png" in client.get("/").data
